...
const Movies = () => {
    ...
    const fetchDetails = async () => {
      const data = await fetch(
        "http://api.tvmaze.com/search/shows?q=drama"
      )
  
      const dramaData = await data.json()
      console.log("ini datanya => ", dramaData);
      setDrama(dramaData)
    }
  ...
  return (
    <div className="container movies__wrapper" >
        <h3 className='movies__recommend'>Drama recommendation</h3>
        <Splide options={{
            perPage : 6,
            breakpoints: {
              428: {  
